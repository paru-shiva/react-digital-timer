...
class DigitalTimer extends Component {
  ...
  resumeClock = () => {
    this.setState({isClockStarted: true, isTimeReset: false})
    const {currentTime, timeSet, timerId} = this.state
    if (currentTime === 0) {
      this.setState({currentTime: timeSet*60})
  ...
  decreaseTime = () => {
    const {isTimeReset, timeSet} = this.state
    if (isTimeReset && timeSet > 1) {
      this.setState(prevState => ({
        currentTime: prevState.currentTime - 60,
        timeSet: prevState.timeSet - 1,
      }))
